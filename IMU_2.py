...
while(time.time() - time1 < 25):
    ...
    if(prev == 1 and ans == 0 and cunt > 20):
        cunt = 0
        steps = steps + 1
        Threshold = gpd.reset()
    ...
    if(prev == 0 and ans == 1):
        ...
        #tcp_send('192.168.43.32',7800,"S%s,%s:1"%(position[0],position[1]))
        #velocity=np.zeros(3)
    if(cunt > 40):
        print("stop")
        velocity = np.array([0,0,0])
    ...
    if(time.time()-time_sample < 10.0):
        j = j + 1
        print(j,yaw)

    else:
        #print("time  :  ",time.time() - time1,yaw_temp,yaw)
        print(yaw)
        del_t = 10.0/j
        if(flag3 == 0):
...
print(i,(np.linalg.norm(position[:2])*9.3)/6.25,np.linalg.norm(position[:2]))
